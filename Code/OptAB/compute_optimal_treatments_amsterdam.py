...
with open('C:/Users/wendland/Documents/Amsterdamdata/sepsis_all_1_lab_amsterdam.pkl', 'rb') as handle:
    data_amsterdam = pickle.load(handle)

with open('C:/Users/wendland/Documents/Amsterdamdata/sepsis_all_1_keys_amsterdam.pkl', 'rb') as handle:
    keys_amsterdam = pickle.load(handle)
key_times_amsterdam = keys_amsterdam

with open('C:/Users/wendland/Documents/Amsterdamdata/sepsis_all_1_variables_amsterdam.pkl', 'rb') as handle:
    variables_amsterdam = pickle.load(handle)

with open('C:/Users/wendland/Documents/Amsterdamdata/sepsis_all_1_variables_complete_amsterdam.pkl', 'rb') as handle:
    variables_complete_amsterdam = pickle.load(handle)

with open('C:/Users/wendland/Documents/Amsterdamdata/sepsis_all_1_static_amsterdam.pkl', 'rb') as handle:
    static_tensor_amsterdam = pickle.load(handle)

with open('C:/Users/wendland/Documents/Amsterdamdata/sepsis_all_1_static_variables_amsterdam.pkl', 'rb') as handle:
    static_variables_amsterdam = pickle.load(handle)

with open('C:/Users/wendland/Documents/Amsterdamdata/lab_amsterdam_mimicvars.pkl', 'rb') as handle:
    lab = pickle.load(handle)
    
with open('C:/Users/wendland/Documents/Amsterdamdata/patient_ids_threeab.pkl', 'rb') as handle:
    onlythree = pickle.load(handle)
    
with open('C:/Users/wendland/Documents/Amsterdamdata/sepsis_all_1_indices_train_amsterdam.pkl', 'rb') as handle:
    indices_train_amsterdam = pickle.load(handle)
    
with open('C:/Users/wendland/Documents/Amsterdamdata/sepsis_all_1_indices_test_amsterdam_amsterdam.pkl', 'rb') as handle:
    indices_test_amsterdam = pickle.load(handle)

sepsis3 = pd.read_csv('C:/Users/wendland/Documents/Amsterdamdata/sepsis3_def.csv')

creat_dat = lab[lab['item']=='Creatinine']
...
model = utils_paper.NeuralCDE(input_channels=data_amsterdam.shape[2], hidden_channels=hidden_channels, hidden_states=hidden_states, output_channels=4, treatment_options=3, activation = activation, num_depth=num_depth, interpolation="linear", pos=True, thresh=data_thresh, pred_comp=pred_comp, pred_act=pred_act, pred_states=pred_states, pred_depth=pred_depth,static_dim=len(static_variables_amsterdam))
...
z0_hidden_dimension_dec = hidden_channels + 1 + static_tensor_amsterdam.shape[-1] + 6
...
## "Cutting" key_times_amsterdam after last observed timepoint of TRAINING (!) data (in this split similar)
data_active_overall = (~data_amsterdam[:,list(key_times_amsterdam.values()),1:2].isnan()[indices_test_amsterdam])
key_times_amsterdam_index = np.array(list(key_times_amsterdam.keys()))[:len(data_active_overall[:,:,0].any(0)) - list(data_active_overall[:,:,0].any(0))[::-1].index(True)]
key_times_amsterdam_train={list(key_times_amsterdam.keys())[x]: key_times_amsterdam[x] for x in key_times_amsterdam_index}
key_times_amsterdam=key_times_amsterdam_train

#Extracting outcome and side-effects from data and setting device
data_X_test = data_amsterdam[:,list(key_times_amsterdam.values())[1:],1:2][indices_test_amsterdam].to(model.device)
data_toxic=data_amsterdam[:,list(key_times_amsterdam.values())[1:],[i=="creatinine" for i in variables_complete]]
data_toxic=data_toxic[:,:,None][indices_test_amsterdam].to(model.device)
data_toxic2=data_amsterdam[:,list(key_times_amsterdam.values())[1:],[i=="bilirubin_total" for i in variables_complete]]
data_toxic2=data_toxic2[:,:,None][indices_test_amsterdam].to(model.device)
data_toxic3=data_amsterdam[:,list(key_times_amsterdam.values())[1:],[i=="alt" for i in variables_complete]]
data_toxic3=data_toxic3[:,:,None][indices_test_amsterdam].to(model.device)
...
data_treatment=data_amsterdam[:,list(key_times_amsterdam.values()),[i=="Vancomycin" for i in variables_complete]]
data_treatment=data_treatment[:,:,None][indices_test_amsterdam].to(model.device)
data_treatment2=data_amsterdam[:,list(key_times_amsterdam.values()),[i=="Piperacillin-Tazobactam" for i in variables_complete]]
data_treatment2=data_treatment2[:,:,None][indices_test_amsterdam].to(model.device)
data_treatment3=data_amsterdam[:,list(key_times_amsterdam.values()),[i=="Ceftriaxon" for i in variables_complete]]
data_treatment3=data_treatment3[:,:,None][indices_test_amsterdam].to(model.device)
data_treatment_test = torch.cat([data_treatment,data_treatment2,data_treatment3],axis=-1)

#Extracting the covariables
data_covariables_test = data_amsterdam[:,:list(key_times_amsterdam.keys())[-1],:].clone()[indices_test_amsterdam].to(model.device)

#Normalizing the missing masks to one
time_max = data_amsterdam.shape[1]
...
data_time_test = data_amsterdam[:,:list(key_times_amsterdam.keys())[-1],0:1][indices_test_amsterdam].to(model.device)
data_active_test = ~data_amsterdam[:,list(key_times_amsterdam.values()),1:2].isnan()[indices_test_amsterdam].to(model.device)
data_static_test=static_tensor_amsterdam[indices_test_amsterdam].to(model.device,dtype=torch.float32)    

# List of all Antibiotic combinations
treatment_options_string = list(powerset(['Vancomycine','Piperacilline (Pipcil)','Ceftriaxon (Rocephin)']))[1:-1]
...
for k in range(data_X_test.shape[0]):
    ...
    if k==189:
        # one dosage of ceftriaxone for this patient...
        # sepsis3[sepsis3['admissionid']==11194]
        # pat_onlythree[pat_onlythree['admissionid']==11194]
        first_ab_tp = torch.tensor([20])
    else:
    ...
    if data_static_test[k,static_variables_amsterdam.index('male')]==1:
        se = sideeffects_m
    else:
        se = sideeffects_f 
    ...
    sepsis_start = max(int(sepsis3[sepsis3['admissionid']==onlythree[k]]['measuredat']),0)
    ...
    while ~np.isnan(start_pred) and ~data_covariables_test[k,start_pred,1].isnan():
        ...
        creat_dat_k = creat_dat[creat_dat['admissionid']==onlythree[k]]
        creat_dat_start_pred = creat_dat_k[creat_dat_k['measuredat']<=start_pred+sepsis_start]
        ...
        for i in range(len(treatment_options_string)):
            ...
            elif not viol and (((bil_k>se[1]).any() or (pred_X_val_unsc[:,:,2]>se[1]).any() or (alt_k>se[2]).any() or (pred_X_val_unsc[:,:,3]>se[2]).any()) and ('Ceftriaxon (Rocephin)' in treat_opt_str)):# or 'Piperacillin-Tazobactam' in treat_opt_str)):
                violated_sol.append(i)
                violated_res.append(pred_X_val_unsc)
                viol=True
                print('bil or alt')
        ...
        creat_dat_k = creat_dat[creat_dat['admissionid']==onlythree[k]]
        creat_dat_start_pred = creat_dat_k[creat_dat_k['measuredat']<=start_pred+sepsis_start+pred_horizon]
        ...
        if (cr[start_pred:]>se[0]).any() and ('Vancomycine' in best_treat):# or 'Piperacillin-Tazobactam' in treat_opt_str)):
        ...
        if (bil[start_pred:]>se[1]).any() and ('Ceftriaxon (Rocephin)' in best_treat):# or 'Piperacillin-Tazobactam' in treat_opt_str)):
        ...
        if (alt[start_pred:]>se[2]).any() and ('Ceftriaxon (Rocephin)' in best_treat):# or 'Piperacillin-Tazobactam' in treat_opt_str)):
        ...
        if 'Piperacilline (Pipcil)' in best_treat:
            if break_time is None:
        ...
        if 'Ceftriaxon (Rocephin)' in best_treat:
            if break_time is None:
...
with open('/work/wendland/opt_treat_amsterdam/feasible_sol_list_all_corrected.pkl', 'wb') as handle:
    pickle.dump(feasible_sol_list_all, handle, protocol=pickle.HIGHEST_PROTOCOL)
    
with open('/work/wendland/opt_treat_amsterdam/sofa_min_arg_list_all_corrected.pkl', 'wb') as handle:
    pickle.dump(sofa_min_arg_list_all, handle, protocol=pickle.HIGHEST_PROTOCOL)
    
with open('/work/wendland/opt_treat_amsterdam/best_treat_list_all_corrected.pkl', 'wb') as handle:
    pickle.dump(best_treat_list_all, handle, protocol=pickle.HIGHEST_PROTOCOL)
    
with open('/work/wendland/opt_treat_amsterdam/sofa_min_list_all_corrected.pkl', 'wb') as handle:
    pickle.dump(sofa_min_list_all, handle, protocol=pickle.HIGHEST_PROTOCOL)
    
with open('/work/wendland/opt_treat_amsterdam/feasible_tensors_list_all_corrected.pkl', 'wb') as handle:
    pickle.dump(feasible_tensors_list_all, handle, protocol=pickle.HIGHEST_PROTOCOL)
   
with open('/work/wendland/opt_treat_amsterdam/best_treat_int_list_all_corrected.pkl', 'wb') as handle:
    pickle.dump(best_treat_int_list_all, handle, protocol=pickle.HIGHEST_PROTOCOL)

with open('/work/wendland/opt_treat_amsterdam/opt_time_list_all_corrected.pkl', 'wb') as handle:
...
for i in range(creat_val.shape[0]):
    ...
    if data_static_test[i,static_variables_amsterdam.index('male')]==1:
        bil_se = 2.4
    else:
        bil_se = 2.2
